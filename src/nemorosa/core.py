...
class NemorosaCore:
    ...
    def __init__(self, torrent_client: TorrentClient, target_apis: list[dict]):
    ...
    def scan(
        ...
        *,
        ...
        if tid == -1:
            ...
            self.database.add_scan_result(scan_source, local_torrent_name, tid, site_host)
            downloaded = False
        else:
            self.stats["found"] += 1
            self.logger.success(f"Found match! Torrent ID: {tid}")
            ...
            if not downloaded:
                ...
                self.database.add_undownloaded_torrent(tid, torrent_info, site_host)
    ...
    def process_single_torrent_from_client(
        ...
        for api_info in self.target_apis:
            self.logger.debug(f"Trying target site: {api_info['server']} (tracker: {api_info['tracker']})")

            # Check if this content already exists on current target tracker
            if api_info["tracker"] in existing_target_trackers:
                self.logger.debug(f"Content already exists on {api_info['tracker']}, skipping")
                continue

            try:
                # Scan and match
                tid, downloaded = self.scan(
                    fdict=fdict,
                    tsize=tsize,
                ...
                if tid != -1:
                    any_success = True
                    self.logger.success(f"Successfully processed on {api_info['server']}")

            except Exception as e:
                self.logger.error(f"Error processing torrent on {api_info['server']}: {e}")
                continue
    ...
    def process_torrents(self):
        ...
        target_trackers = [api_info["tracker"] for api_info in self.target_apis if api_info["tracker"]]
    ...
    def retry_undownloaded_torrents(self):
        ...
        try:
            ...
            for api_info in self.target_apis:
                ...
                self.logger.debug(f"Processing undownloaded torrents for site: {api_info['server']}")
                ...
                if not undownloaded_torrents:
                    self.logger.debug(f"No undownloaded torrents found for site: {api_info['server']}")
                    continue

                self.logger.info(f"Found {len(undownloaded_torrents)} undownloaded torrents for site: {api_info['server']}")
